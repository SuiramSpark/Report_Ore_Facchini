...
const SheetEditor = ({ 
    ...
    // FUNZIONI HELPER MANCANTI - AGGIUNTE
    const showToast = (message, type = 'info') => {
        // Implementazione base del toast - puoi sostituire con la tua libreria
    ...
    const generatePDF = async (sheet, logo) => {
        console.log('Generating PDF for:', sheet.titoloAzienda);
        showToast('📄 PDF generato con successo!', 'success');
        // Qui implementerai la generazione PDF effettiva
        return Promise.resolve();
    };

    const generateShareLink = (sheetId) => {
        const baseUrl = window.location.origin;
        const shareUrl = `${baseUrl}/worker-sign/${sheetId}`;
